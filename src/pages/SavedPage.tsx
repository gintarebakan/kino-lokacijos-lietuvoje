...
export default function SavedPage() {
  ...
  const { bookmarks, routeLocations, removeBookmark, addToRoute, removeFromRoute, reorderRoute } =
    useSavedStore();
  ...
  const mapsUrl =
    "https://www.google.com/maps/dir/" + routeLocations.map((l) => l.lat + "," + l.lng).join("/");
  ...
  return (
    ...
    >
      <div style={{ display: "flex", borderBottom: "1px solid #222222", flexShrink: 0 }}>
        <button type="button" style={tabStyle("saved")} onClick={() => setActiveTab("saved")}>
          Išsaugota ({bookmarks.length})
        </button>
        <button type="button" style={tabStyle("route")} onClick={() => setActiveTab("route")}>
          Maršrutas ({routeLocations.length})
        </button>
      </div>
            ...
            {routeLocations.length === 0 ? (
              ...
              routeLocations.map((loc, idx) => (
                <div
                  ...
                  onDrop={() => {
                    if (dragIdx !== null && dragIdx !== idx) reorderRoute(dragIdx, idx);
                    setDragIdx(null);
                    setOverIdx(null);
                  }}
                  onDragEnd={() => {
                  ...
                  style={{
                    ...
                    padding: "10px 16px",
                    borderBottom: "1px solid #1a1a1a",
                    background: overIdx === idx ? "rgba(201,168,76,0.08)" : "transparent",
                  }}
                >
                  <div
                    style={{
          ...
          {routeLocations.length >= 2 && (
            ...
            >
                ...
                {(["driving", "walking"] as const).map((mode) => (
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 12,
                      fontWeight: 600,
                      background: travelMode === mode ? "#c9a84c" : "#1a1a1a",
                      color: travelMode === mode ? "#0a0a0a" : "#9ca3af",
                      border: "none",
                      cursor: "pointer",
                    }}
                  >
              ...
              {routeInfo && !routeLoading && (
                <div
                  style={{ color: "#c9a84c", fontSize: 13, textAlign: "center", fontWeight: 600 }}
                >
                  {routeInfo}
                </div>
              )}
